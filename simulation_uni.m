function simulation_uni(UE_num,time,dropnum,dropout,K,mode)
...
%Update worst SINR
    for index = 1:7
        ...
        for i = 1:7
            ...
            throughput = sum(member_num.*R); %%%BUGBUGBUG
            worstSINR2((worstSINR2==inf)) = [];
            if numel(worstSINR2)>0
                R2 = rate(10.^(worstSINR2./10),bw);
                resource = sum(1./R2);
                efficiency = efficiency+sum(throughput/resource);
            end
        end       
    end
    total_eff = total_eff+efficiency/7;
end
disp('----------------------REPORT----------------------')
...
%UE.SINR
average_efficiency = total_eff/time;
average_efficiency
Regroup_count
%pingpongarray(1)
